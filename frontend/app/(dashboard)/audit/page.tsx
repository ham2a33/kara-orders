  ...
  return (
    <div className="flex flex-col gap-6">
      <section className="space-y-2">
        <SectionBadge>Аудит</SectionBadge>
        <h1 className="text-3xl font-semibold tracking-tight">Операционный аудит</h1>
        <p className="max-w-2xl text-sm text-muted-foreground">
          Отслеживайте входы, смену ролей, изменения компании, AI-запросы, заказы и обновления товаров.
        </p>
      </section>
      ...
      <Card>
        <CardContent className="grid gap-4 p-5 md:grid-cols-[1fr_auto]">
          <label className="grid gap-2 text-sm">
            <span className="text-muted-foreground">Фильтр по действию</span>
            <input
              value={action}
      ...
      <Panel title="Последняя активность" description="Серверный аудит с изоляцией компаний.">
              ...
              <div className="flex flex-wrap items-start justify-between gap-3">
                <div>
                  ...
                  <p className="mt-1 text-sm text-muted-foreground">
                    {log.description ?? "Без описания"} • {formatDate(log.created_at)}
                  </p>
                </div>
        ...
        <div className="mt-4 flex items-center justify-between">
          <p className="text-sm text-muted-foreground">
            Страница {query.data?.page ?? page} из {Math.max(Math.ceil((query.data?.total ?? 0) / 20), 1)}
          </p>
